...
export class ProductsController{
    ...
    static async addProduct(req,res){
        try {
            ...
            const userId = await UsersService.getUserByEmail(req.body.email);
            const newProduct = req.body
            newProduct.owner = userId._id;
            const productSaved = await ProductsService.addProduct(newProduct);
            return res.json({status:"success", message:"Producto creado", data:productSaved}); //Mensaje de exito de producto creado
        } catch (error) {
            return res.status(500).send({status:
                customError.createError({
